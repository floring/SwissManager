...
public class MatchesCreator {
    ...
    public List<Match> createMatchList(List<Player> players) {
        int[] playresOrder = orderBySwissSystem(players);
        List<Match> matches = makeMatches(playresOrder);

        return matches;
    }

    private int[] orderBySwissSystem(List<Player> players) {
        ...
        while (pg.hasMore()) {
            int[] currPlayersOrder = pg.getNext();
            ...
            for (int i = 0; i < playersNum / 2; i++) {
                ...
                if (hadPlayed) {
                    orderHealth += Math.abs(players.indexOf(players.get(currPlayersOrder[i * 2])) -
            ...
            if (playersNum % 2 != 0) {
                if (players.get(currPlayersOrder[playersNum - 1]).hadBye()) {
                    ...
                    orderHealth += playersNum - players.indexOf(players.get(currPlayersOrder[playersNum - 1]));
                }
            }
            // if good, update
    ...
    private List<Match> makeMatches(int[] playerOrder) {
        Player byePlayer;
        List<Match> matches
